import {
  AboutSection,
  HeroButton,
  MyProject,
  SkillsSection,
} from "@/components";
import Link from "next/link";
import React from "react";

const index = () => {
  return (
    <div className="container h-screen">
      <div className="container h-screen bg-[url('/bg.jpg')]  ">
        <div className="h-16 flex w-full  items-center ">
          <div className="flex items-center justify-between  w-full px-48">
            <div className="w-fit">
              <img src="./chinmayLogo.png" alt="img" className="h-10 w-16" />
            </div>
            <div className="space-x-11">
              <Link
                href={"./about"}
                className="font-bold text-black hover:text-[#5F00D3]"
              >
                About Me
              </Link>
              <Link
                href={"./Experience"}
                className="font-bold text-black hover:text-[#5F00D3]"
              >
                Experience
              </Link>
              <Link
                href={"./Skill"}
                className="font-bold text-black hover:text-[#5F00D3]"
              >
                Skills
              </Link>
              <Link
                href={"./contact"}
                className="font-bold text-black hover:text-[#5F00D3]"
              >
                Contact Me
              </Link>
              <Link
                href={"./resume"}
                className="font-bold text-black hover:text-[#5F00D3]"
              >
                Project
              </Link>
            </div>
            <div className="buttonContainer w-fit">
              <a href="#" className="button">
                Download CV
              </a>
            </div>
          </div>
        </div>
        <div className="h-5/6 flex w-full items-center">
          <div className="px-48 flex justify-between w-full items-center">
            <div className="w-1/2  items-start flex flex-col gap-2">
              <h3 className="text-[#5F00D3]  font-semibold text-3xl">
                Hello, I'm
              </h3>
              <h6 className="text-5xl font-bold">Chinmay Muduli</h6>
              <p className="space-x-1 font-semibold mt-1">
                A{" "}
                <span className="font-semibold text-[#1cbe59]">
                  Mobile Developer
                </span>{" "}
                from<span className="font-semibold text-[#f75023]">india</span>
              </p>
              <h6 className="font-semibold text-[#6f6b80] mt-3 w-[500px]">
                I'm Mobile Developer based in India, and i'm very passionate and
                dedicated to my work.
              </h6>
              <button className="btn-8 mt-3">About Me</button>
            </div>
            <div className="w-1/2 flex items-center justify-center relative">
              <img
                src="./avatar.png"
                alt="img"
                className="h-96 w-80 rounded-full"
              />
              <span className="animate_btn bg-white absolute top-[1.1rem] rounded-lg h-[50px] w-[50px] flex justify-center items-center left-[7rem] shadow-lg ">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  viewBox="0 0 384 512"
                  height="35px"
                  width="35px"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z"></path>
                </svg>
              </span>
              <span className="animate_btn bg-red-600  absolute top-[5.25rem] right-[7rem] rounded-lg h-[50px] w-[50px] flex justify-center items-center  shadow-lg">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  version="1.1"
                  x="0px"
                  y="0px"
                  viewBox="0 0 48 48"
                  enable-background="new 0 0 48 48"
                  height="35px"
                  width="35px"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g>
                    <path
                      fill="#7CB342"
                      d="M12,29.001c0,1.104-0.896,2-2,2l0,0c-1.104,0-2-0.896-2-2v-9c0-1.104,0.896-2,2-2l0,0c1.104,0,2,0.896,2,2
		V29.001z"
                    ></path>
                    <path
                      fill="#7CB342"
                      d="M40,29.001c0,1.104-0.896,2-2,2l0,0c-1.104,0-2-0.896-2-2v-9c0-1.104,0.896-2,2-2l0,0c1.104,0,2,0.896,2,2
		V29.001z"
                    ></path>
                    <path
                      fill="#7CB342"
                      d="M22,40c0,1.104-0.896,2-2,2l0,0c-1.104,0-2-0.896-2-2v-9c0-1.104,0.896-2,2-2l0,0c1.104,0,2,0.896,2,2V40z"
                    ></path>
                    <path
                      fill="#7CB342"
                      d="M30,40c0,1.104-0.896,2-2,2l0,0c-1.104,0-2-0.896-2-2v-9c0-1.104,0.896-2,2-2l0,0c1.104,0,2,0.896,2,2V40z"
                    ></path>
                    <path
                      fill="#7CB342"
                      d="M14,18.001V33c0,1.104,0.896,2,2,2h16c1.104,0,2-0.896,2-2V18.001H14z"
                    ></path>
                    <path
                      fill="#7CB342"
                      d="M24,8c-6,0-9.655,3.645-10,8h20C33.654,11.645,30,8,24,8z M20,13.598c-0.552,0-1-0.448-1-1s0.448-1,1-1
		s1,0.448,1,1S20.552,13.598,20,13.598z M28,13.598c-0.553,0-1-0.448-1-1s0.447-1,1-1s1,0.448,1,1S28.553,13.598,28,13.598z"
                    ></path>
                    <line
                      fill="none"
                      stroke="#7CB342"
                      stroke-width="2"
                      stroke-linecap="round"
                      x1="30"
                      y1="7"
                      x2="28.334"
                      y2="9.499"
                    ></line>
                    <line
                      fill="none"
                      stroke="#7CB342"
                      stroke-width="2"
                      stroke-linecap="round"
                      x1="18"
                      y1="7"
                      x2="19.333"
                      y2="9.082"
                    ></line>
                  </g>
                </svg>
              </span>
              <span className="animate_btn bg-[#3E7ADB]  absolute left-[8rem] bottom-[3.7rem] rounded-lg h-[50px] w-[50px] flex justify-center items-center  shadow-lg">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  role="img"
                  viewBox="0 0 24 24"
                  height="35px"
                  width="35px"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <title></title>
                  <path d="M14.314 0L2.3 12 6 15.7 21.684.012h-7.357L14.314 0zm.014 11.072l-6.471 6.457 6.47 6.47H21.7l-6.46-6.468 6.46-6.46h-7.371z"></path>
                </svg>
              </span>
              <span className="animate_btn bg-[#61DAFB] absolute bottom-2 right-[8.5rem] rounded-lg h-[50px] w-[50px] flex justify-center items-center  shadow-lg">
                <svg
                  stroke="currentColor"
                  fill="currentColor"
                  stroke-width="0"
                  viewBox="0 0 512 512"
                  height="35px"
                  width="35px"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M418.2 177.2c-5.4-1.8-10.8-3.5-16.2-5.1.9-3.7 1.7-7.4 2.5-11.1 12.3-59.6 4.2-107.5-23.1-123.3-26.3-15.1-69.2.6-112.6 38.4-4.3 3.7-8.5 7.6-12.5 11.5-2.7-2.6-5.5-5.2-8.3-7.7-45.5-40.4-91.1-57.4-118.4-41.5-26.2 15.2-34 60.3-23 116.7 1.1 5.6 2.3 11.1 3.7 16.7-6.4 1.8-12.7 3.8-18.6 5.9C38.3 196.2 0 225.4 0 255.6c0 31.2 40.8 62.5 96.3 81.5 4.5 1.5 9 3 13.6 4.3-1.5 6-2.8 11.9-4 18-10.5 55.5-2.3 99.5 23.9 114.6 27 15.6 72.4-.4 116.6-39.1 3.5-3.1 7-6.3 10.5-9.7 4.4 4.3 9 8.4 13.6 12.4 42.8 36.8 85.1 51.7 111.2 36.6 27-15.6 35.8-62.9 24.4-120.5-.9-4.4-1.9-8.9-3-13.5 3.2-.9 6.3-1.9 9.4-2.9 57.7-19.1 99.5-50 99.5-81.7 0-30.3-39.4-59.7-93.8-78.4zM282.9 92.3c37.2-32.4 71.9-45.1 87.7-36 16.9 9.7 23.4 48.9 12.8 100.4-.7 3.4-1.4 6.7-2.3 10-22.2-5-44.7-8.6-67.3-10.6-13-18.6-27.2-36.4-42.6-53.1 3.9-3.7 7.7-7.2 11.7-10.7zM167.2 307.5c5.1 8.7 10.3 17.4 15.8 25.9-15.6-1.7-31.1-4.2-46.4-7.5 4.4-14.4 9.9-29.3 16.3-44.5 4.6 8.8 9.3 17.5 14.3 26.1zm-30.3-120.3c14.4-3.2 29.7-5.8 45.6-7.8-5.3 8.3-10.5 16.8-15.4 25.4-4.9 8.5-9.7 17.2-14.2 26-6.3-14.9-11.6-29.5-16-43.6zm27.4 68.9c6.6-13.8 13.8-27.3 21.4-40.6s15.8-26.2 24.4-38.9c15-1.1 30.3-1.7 45.9-1.7s31 .6 45.9 1.7c8.5 12.6 16.6 25.5 24.3 38.7s14.9 26.7 21.7 40.4c-6.7 13.8-13.9 27.4-21.6 40.8-7.6 13.3-15.7 26.2-24.2 39-14.9 1.1-30.4 1.6-46.1 1.6s-30.9-.5-45.6-1.4c-8.7-12.7-16.9-25.7-24.6-39s-14.8-26.8-21.5-40.6zm180.6 51.2c5.1-8.8 9.9-17.7 14.6-26.7 6.4 14.5 12 29.2 16.9 44.3-15.5 3.5-31.2 6.2-47 8 5.4-8.4 10.5-17 15.5-25.6zm14.4-76.5c-4.7-8.8-9.5-17.6-14.5-26.2-4.9-8.5-10-16.9-15.3-25.2 16.1 2 31.5 4.7 45.9 8-4.6 14.8-10 29.2-16.1 43.4zM256.2 118.3c10.5 11.4 20.4 23.4 29.6 35.8-19.8-.9-39.7-.9-59.5 0 9.8-12.9 19.9-24.9 29.9-35.8zM140.2 57c16.8-9.8 54.1 4.2 93.4 39 2.5 2.2 5 4.6 7.6 7-15.5 16.7-29.8 34.5-42.9 53.1-22.6 2-45 5.5-67.2 10.4-1.3-5.1-2.4-10.3-3.5-15.5-9.4-48.4-3.2-84.9 12.6-94zm-24.5 263.6c-4.2-1.2-8.3-2.5-12.4-3.9-21.3-6.7-45.5-17.3-63-31.2-10.1-7-16.9-17.8-18.8-29.9 0-18.3 31.6-41.7 77.2-57.6 5.7-2 11.5-3.8 17.3-5.5 6.8 21.7 15 43 24.5 63.6-9.6 20.9-17.9 42.5-24.8 64.5zm116.6 98c-16.5 15.1-35.6 27.1-56.4 35.3-11.1 5.3-23.9 5.8-35.3 1.3-15.9-9.2-22.5-44.5-13.5-92 1.1-5.6 2.3-11.2 3.7-16.7 22.4 4.8 45 8.1 67.9 9.8 13.2 18.7 27.7 36.6 43.2 53.4-3.2 3.1-6.4 6.1-9.6 8.9zm24.5-24.3c-10.2-11-20.4-23.2-30.3-36.3 9.6.4 19.5.6 29.5.6 10.3 0 20.4-.2 30.4-.7-9.2 12.7-19.1 24.8-29.6 36.4zm130.7 30c-.9 12.2-6.9 23.6-16.5 31.3-15.9 9.2-49.8-2.8-86.4-34.2-4.2-3.6-8.4-7.5-12.7-11.5 15.3-16.9 29.4-34.8 42.2-53.6 22.9-1.9 45.7-5.4 68.2-10.5 1 4.1 1.9 8.2 2.7 12.2 4.9 21.6 5.7 44.1 2.5 66.3zm18.2-107.5c-2.8.9-5.6 1.8-8.5 2.6-7-21.8-15.6-43.1-25.5-63.8 9.6-20.4 17.7-41.4 24.5-62.9 5.2 1.5 10.2 3.1 15 4.7 46.6 16 79.3 39.8 79.3 58 0 19.6-34.9 44.9-84.8 61.4zm-149.7-15c25.3 0 45.8-20.5 45.8-45.8s-20.5-45.8-45.8-45.8c-25.3 0-45.8 20.5-45.8 45.8s20.5 45.8 45.8 45.8z"></path>
                </svg>
              </span>
            </div>
          </div>
        </div>
        <div className="absolute left-1/2 right-1/2 bottom-0  ">
          <HeroButton />
        </div>
      </div>
      <div>
        <AboutSection />
      </div>
      <SkillsSection />
      <MyProject />
    </div>
  );
};

export default index;
